...
def get_file_path(interface_name):
    file_path  =  INTERFACE_FILE_NAME_MAP.get(interface_name)
    if not file_path:
...
def get_func_name_interface_name_from_func_call(func_call: str):
    ...
    regex_result  =  re.compile('s\.([0-9a-zA-Z\_]+)(Utils\.|Dao\.|ES\.|Service\.|Scripts\.)(.*)\(').findall(func_call)
...
def get_func_call_statement(mock_func_call_file_path: str, mock_func_call_file_line_number: int, mock_func_name: str):
    mock_func_call_file_contents  =  read_file_contents(mock_func_call_file_path)
    ...
    for line in mock_func_call_lines:
        line  =  str(line).strip()
        if line and mock_func_name in line and re.search(REGEX_OTHER_INTERFACE_FUNC_CALL, line):
            return line

# def get_interface_name(mock_func_call_file_path: str, mock_func_call_file_line_number: int, mock_func_name: str):
#             _, called_interface_name  =  get_func_name_interface_name_from_func_call()
#                 _, called_interface_name  =  get_func_name_interface_name_from_func_call(line)
    
#     if called_interface_name  ==  "":
#         logging.error(f"unhandled: called_interface_name {mock_func_name} not found in file: " + mock_func_call_file_path, exc_info  =  True)
#         raise Exception(f"unhandled: called_interface_name {mock_func_name} not found in file: " + mock_func_call_file_path)            
#     return called_interface_name
    ...
    for i in range(len(unexpected_method_call_input_params)):
        if unexpected_method_call_input_params[i][3:].startswith("resources.FilterInput{SearchText:"):
            ...
            if "(*datatypes.JSON)(" in mock_func.mock_func_inputs[i]:
                for datatypesJson in list(set(re.compile("(\(\*datatypes\.JSON\)\(0x[0-9a-f]*\))").findall(mock_func.mock_func_inputs[i]))):
            ...
            if "(*uint64)(0x" in mock_func.mock_func_inputs[i]:
                for ptr_with_address in list(set(re.compile("(\(\*uint64\)\([0-9a-fx]*\))").findall(mock_func.mock_func_inputs[i]))):
            ...
            if "0x" in mock_func.mock_func_inputs[i]:
                for hex in list(set(re.compile("(0x[0-9a-f]*)").findall(mock_func.mock_func_inputs[i]))):
...
def form_import_statements(file_content):
    ...
    if "tests." in file_content:
        import_statements.append('"'+CWD+'/tests"')
    if "resources." in file_content:
        import_statements.append('"'+CWD+'/resources"')
    if "models." in file_content:
        import_statements.append('"'+CWD+'/models"')
...
def form_ut_test_cases(ut_test_case_dict: UT_TEST_CASES_DICT):
    ut_test_cases  =  []
...
def replace_expected_outputs(expected_outputs:List, index:int, actual:str):
    ...
    if "&errors.errorString{s:" in actual:
        ...
        for ptr_with_address in list(set(re.compile("(\(\*uint64\)\([0-9a-fx]*\))").findall(actual))):
            actual  =  actual.replace(ptr_with_address, "Uint64Ptr")
    elif "0x" in actual:
        for hex in list(set(re.compile("(0x[0-9a-f]*)").findall(actual))):
...
def output_assertion(test_case: TEST_CASE_DICT, failed_test_case: str):
    ...
    output_assert_failed_index  =  int(list(set(re.compile("Not equal output param - out(.*), kindly check").findall(failed_test_case)))[0].strip())
...
def get_file_path_by_struct(type: str, struct_name: str):
    file_path  =  STRUCT_NAME_FILE_MAP[type].get(struct_name)
    if not file_path:
...
def get_struct_name_type(file_path: str, struct_name: str):
    file_contents  =  read_file_contents(path = file_path).replace('\t', '')
    ...
    for i in range(len(structs)):
        ...
        if struct_pattern in struct_content:
            ...
            var_name_type  =  list(set(re.compile("([a-zA-Z0-9]*)\s*([a-zA-Z0-9\[\]\.]*).+").findall(primary_key)))[0]
...
def other_possible_arg(output_param: str):
    if output_param.startswith("fmt.Errorf"):
        return ["nil"]
    # if output_param.startswith("error"):
    #     return ["nil"]
...
def mock_unexpected_method_call(test_case_q: List[TEST_CASE_DICT], ut_test_case_dict: UT_TEST_CASES_DICT, test_case: TEST_CASE_DICT, mock_func_name: MOCK_FUNC_DICT, failed_test_case: str) -> MOCK_FUNC_DICT:
    mock_file_path  =  list(set(re.compile("(at: \[)(.*)(\] \[recovered\])").findall(failed_test_case)))[0][1].strip().split(' ')[1].strip()
...
def method_cal_was_unexpected(test_case_q: List[TEST_CASE_DICT], ut_test_case_dict: UT_TEST_CASES_DICT, test_case: TEST_CASE_DICT, failed_test_case: str):
    to_mock_func_list  =  list(set(re.compile('Either do Mock.On\(\"(.*)\"\).Return\(\.\.\.\)').findall(failed_test_case)))
...
def run_test_case(func_name):
    return os.popen("/usr/local/go/bin/go test -timeout 30s -run ^Test%s$ %s/tests/service" % (CWD, func_name)).read()

def write_to_back_up_file(ut_test_case_dict, file_path):
    test_cases  =  form_ut_test_cases(ut_test_case_dict)
    ut_test_cases_file  =  CWD+"/tests/test_cases/auto_generated_test_cases_bck.go"
    write_to_file(test_cases, ut_test_cases_file)
...
def auto_generate_test_cases(test_case_q: List[TEST_CASE_DICT], func_name) -> UT_TEST_CASES_DICT:
    ...
    try:
        ...
        while len(test_case_q):
            ...
            if test_case.test_case_id == 43:
                pass

            tries  =  0
            while tries < 10:
                test_cases  =  form_ut_test_cases(UT_TEST_CASES_DICT(func_name, [test_case]))
                ut_test_cases_file  =  CWD+"/tests/test_cases/auto_generated_test_cases.go"
                write_to_file(test_cases, ut_test_cases_file)
                ...
                if "asserting out" in failed_test_case:
                    ...
                    if "panic: interface conversion: interface {} is nil, not" in failed_test_case:
                        write_to_back_up_file(ut_test_case_dict=ut_test_case_dict, file_path=CWD+"/tests/test_cases/auto_generated_test_cases_bck.go")
                    raise Exception("unhandled: auto_generate")

                # move running test-case to a func
            if tries >=  10:
                write_to_back_up_file(ut_test_case_dict=ut_test_case_dict, file_path=CWD+"/tests/test_cases/auto_generated_test_cases_bck.go")
            ...
            # if test_case.test_case_id  ==  50:
            #     # implement user interface here
            #     logging.error("More than 25 test-cases formed please check", exc_info  =  True)
            #     raise Exception("More than 25 test-cases formed please check")
...
def main():
    os.system("git stash")
    os.system("git checkout ut-nada")
    os.system("rm tests/test_cases/auto_generated_test_cases.go")
    os.system("rm tests/test_cases/auto_generated_test_cases_bck.go")
    old_coverage_contents  =  os.popen("sh coverage.sh").read()
    ...
    for line in old_coverage_contents:
        if "coverage: " in line:
            old_coverage  =  float(list(set(re.compile("coverage: (\d{1,2}|\d{1,2}\.\d{1,2})% of statements in").findall(line)))[0])
    ...
    func_name  =  "ReceivedDemoHandler"
    interface_name  =  "CampaignMemberService"
    path  =  CWD+"/service/v1/campaign_member.go"
    ...
    test_service_file  =  CWD+"/tests/service/campaign_member_test.go"
    append_to_file(test_service, test_service_file)
    ...
    try:
        ...
        ut_test_cases_file  =  CWD+"/tests/test_cases/auto_generated_test_cases.go"
        write_to_file(test_cases, ut_test_cases_file)
        new_coverage_contents  =  os.popen("sh coverage.sh").read()
        ...
        for line in new_coverage_contents:
            if "coverage: " in line:
                new_coverage  =  float(list(set(re.compile("coverage: (\d{1,2}|\d{1,2}\.\d{1,2})% of statements in").findall(line)))[0])
        ...
        print(f"After UT for {func_name}, Covered with Auto Generation {new_coverage-old_coverage}, New coverage {new_coverage}")
        ...
        # ut_test_case_dict.func_name +=  "_BCK"
        # test_cases  =  form_ut_test_cases(ut_test_case_dict)
        # ut_test_cases_file  =  CWD+"/tests/test_cases/auto_generated_test_cases_bck.go"
        # write_to_file(test_cases, ut_test_cases_file)
